...
from basicsr.archs.quant_basicvsr_arch import Quantizable_BasicVSR
...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def test(self):
        ...
        if self.opt['name']=='Quant_BasicVSR_REDS':
            state_dict = torch.load("pretrained_models/BasicVSR/quant_basicvsr_nospynet.pytorch")
            model_int8 = Quantizable_BasicVSR()
            ...
            with torch.no_grad():
                self.lq = self.lq.cpu()
                self.output = model_int8(self.lq)
        ...
        ## 制作 basicvsr_quant.pytorch
        # state_dict = torch.load('pretrained_models/BasicVSR/fixed_basicvsr_model.pytorch')
        #     # state_dict['mean'] = torch.Tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
        #     # state_dict['std'] =  torch.Tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
        # model_fp32 = QuantizableBasicVSR()        # model_fp32.load_state_dict(state_dict)
        # model_fp32.float()
        # model_fp32.eval()
        # model_fp32.qconfig = torch.quantization.get_default_qconfig('fbgemm')
        # model_fp32_prepared = torch.quantization.prepare(model_fp32)
        # data_1 = self.lq.cpu()
        # model_fp32_prepared(data_1)
        # model_int8 = torch.quantization.convert(model_fp32_prepared)
        # torch.save(model_int8.state_dict(),"pretrained_models/BasicVSR/basicvsr_quant.pytorch")
        if flip_seq:
